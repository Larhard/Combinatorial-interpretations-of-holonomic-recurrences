...
Generating functions is a way of describing infinite sequences using formal power series, where coefficients are encoding the the consecutive elements. It allows us to incorporate many advanced methods of analysis of encoded objects.

Every recursive structure can be described using a~generating function equation, which can be then solved and transformed into a~holonomic recurrence, i.e.~recurrence that in order to define \(n\)-th element of the sequence requires just finite number of previous ones and coefficients that are polynomials of \(n\).
...
These interpretations have been source of ideas that that we were incorporating in order to make our partial interpretation for the case of lambda trees in unary de Bruijn notation.
...
To show the idea of such transformations, let's define transformation context with holes. The context specifies shape of the fragment of the tree. It can have holes that specify whole subtree. We can also specify whether the root of the context has parent and is it the left or right child of its parent.
...
We marked the node being transformed to clarify the picture.
...
\begin{definition}
    Section \(T_{i,j}\) is defined as a~subset of \(T_i\) adding a~specific set of nodes. We assign \(j\) according to the lexicographical order of lexicographically sorted node types.
\end{definition}
