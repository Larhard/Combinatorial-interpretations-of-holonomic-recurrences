...
Basicaly, for each virtual node not being root (ie.\ having left or right parent), we point it and add \(l\) or \(s\) respectively above the parent.
...
For each actual vertex, we add \(a\) node as its left or right parent and add as a second child \(o\) node as its siblig. We then point left or right virtual child of newly created \(o\) node.
...
You may also note that we are operating on specific \(N\). The probability that we miss some corner case is low, but even if we check large \(N\) values, it is always present. The formal algebraic proof would make us sure that the construction works. We woulr prioritize finishing the construction first.
