...
Sibling of vertex \(v\) is defined as vertex \(u\) having the same parent as \(v\).

Uncle of vertex \(v\) is defined as sibling of the parent of \(v\).
...
Leaf is a vertex which has no children.
...
\label{section-unary-binary}

Let's think, what would have changed if we introduce unary vertices to the binary case\cite{motzkin}. We will be counting edges. Note that tree having \(n\) edges has exactly \(n + 1\) vertices.

\begin{definition}
    Unary-binary tree is such tree of which any vertex can have either 0, 1 or 2 children.
\end{definition}
...
\begin{definition}
    \label{def-unary-binary-2}
    Left-leaning binary tree\todo{find official name; skewed binary tree is something different} is a binary tree with following constraint added: if a vertex has right child, it also have a left child.
\end{definition}

The bijection between unary-binary and left-leaning binary trees is very simple. We just represent unary vertices as vertices with just left child of left-leaning binary tree. The other cases translate immediately to corresponding ones. Please note that this transformation keeps the size of the tree (ie.~number of edges).

The next step is to make binary trees full. Let's just define virtual leaves.

\begin{definition}
    Virtual leaf is a virtual node added to the vertex in place of non-existing child.
\end{definition}

As we have added \(n + 2\) virtual vertices, we have added as much virtual edges.
...
To present the interpretation in bijective language, we will have to introduce several types of the the virtual leaves (you may want to revisit section \ref{section-notation} Notation):
...
\begin{enumerate}
    ...
    \item Vrt - virtual leaf being right virtual-twinling. We will divide this case into:
        ...
        \end{enumerate}
    \item Vlt - virtual leaf left virtual-twinling. Note that from definition \ref{def-unary-binary-2}, left virtual leave can not have sibling being a actual node. We can divide also this case into:
        \begin{enumerate}
...
Let's start from \(U(n-1)\). Its multiplier is \(2n + 1\). It suggests that we can point any vertex, as in a tree having \(n - 1\) edges, we have \(n\) actual and \(n + 1\) virtual ones.
...
\begin{definition}
    Schröder tree is a tree with no unary vertices.
\end{definition}

Simply speaking, vertex can have zero or strictly more than one children.
...
If we describe some structure we may be tempted to use multiple kinds of vertices, eg.~unary and binary vertices. Having multiple kinds of vertices may cause problems, both in creating holonomic recurrence and finding the interpretation, as we have to keep track of how they can transform.

As the most common trees are binary ones, it may be worth an effort to find the some structure based on binary trees that can represent original structure (see section~\ref{section-unary-binary}).
