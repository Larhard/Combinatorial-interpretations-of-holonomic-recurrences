...
The only thing that's left for us to verify is if we generate all the trees of size \(n + 1\) with pointed leaves. As from the recursion, we know that \(2 (2n + 1) B(n)\) describe trees with a single leaf pointed. It is enough to check if we do not generate any duplicate using \(R_l\) and \(R_r\). It can happen in only 2 ways:
\begin{enumerate}
